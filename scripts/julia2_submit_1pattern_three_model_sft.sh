...
for spec in "${models[@]}"; do
  IFS='|' read -r label llm_conf base_model <<< "${spec}"
  ...
  job_id=$(sbatch --parsable -p "${partition}" --gres="gpu:${gpus}" \
    ...
    --export "ALL,NNGPT_1PATTERN_SFT_RUN_ID=${run_id},NNGPT_1PATTERN_SFT_MODEL_LABEL=${label},NNGPT_1PATTERN_SFT_LLM_CONF=${llm_conf},NNGPT_1PATTERN_SFT_BASE_MODEL=${base_model},NNGPT_1PATTERN_SFT_GEN_NN_PREFIX=rl-bb-test1-${label}-sftcycle,NNGPT_1PATTERN_SFT_NN_PREFIXES=rl-bb-test1,NNGPT_1PATTERN_SFT_NUM_CYCLES=20,NNGPT_1PATTERN_SFT_TEST_NN=30,NNGPT_1PATTERN_SFT_NN_TRAIN_EPOCHS=1,NNGPT_1PATTERN_SFT_NUM_TRAIN_EPOCHS=1,NNGPT_NNEVAL_WORKERS_PER_GPU=1" \
